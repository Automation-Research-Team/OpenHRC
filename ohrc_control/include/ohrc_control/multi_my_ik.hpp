...
public:
  ...
  int nState = 0;
  std::vector<int> iJnt;
  std::vector<std::unique_ptr<MyIK>> myIKs;
  ...
  MultiMyIK(const std::vector<std::string>& base_link, const std::vector<std::string>& tip_link, const std::vector<std::string>& URDF_param, const std::vector<Affine3d>& T_base_world, double _eps = 1e-5,
