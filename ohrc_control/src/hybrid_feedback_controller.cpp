...
void HybridFeedbackController::updateTargetPose(const rclcpp::Time t, KDL::Frame& pose, KDL::Twist& twist) {
  KDL::Frame frame;
  KDL::Twist vel;
...
VectorXd HybridFeedbackController::adaptivePIControl(const rclcpp::Time t, const KDL::Frame& frame, const KDL::Frame& pose, const KDL::Twist& twist) {
  ...
  for (int i = 0; i < 3; i++) {
    double y0 = a * std::exp(-b * (t.seconds() - t0_f[i]));  // remain feedback just after losing physical contact
...
VectorXd HybridFeedbackController::adaptivePIControl(const rclcpp::Time t, const VectorXd& e, const KDL::Twist& twist) {
  const double kp = 1.2, ki = 0.0;  // 1.0*dt;
