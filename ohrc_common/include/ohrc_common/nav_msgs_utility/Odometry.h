...
class Odometry {
public:
  inline void toVector_3dof(nav_msgs::msg::Odometry odom, Eigen::MatrixXd &vector) {
  ...
  inline void toVector_6dof(nav_msgs::msg::Odometry odom, Eigen::MatrixXd &vector) {
    vector(0) = odom.pose.pose.position.x;                                          //[m]
    vector(1) = odom.pose.pose.position.y;                                          //[m]
  ...
  inline void fromVector_3dof(Eigen::MatrixXd vector, nav_msgs::msg::Odometry &odom) {
  ...
  inline void fromVector_6dof(Eigen::MatrixXd vector, nav_msgs::msg::Odometry &odom) {
    odom.pose.pose.position.x = vector(0);  //[m]
    odom.pose.pose.position.y = vector(1);  //[m]
  ...
  inline void transFromOdom(nav_msgs::msg::Odometry odom, geometry_msgs::TransformStamped &trans_base_odom) {
    Eigen::Affine3d T_base_odom;
    tf2::fromMsg(odom.pose.pose, T_base_odom);
    trans_base_odom = tf2::eigenToTransform(T_base_odom);
  }

  inline void transToOdom(nav_msgs::msg::Odometry odom, geometry_msgs::TransformStamped &trans_odom_base) {
    Eigen::Affine3d T_base_odom;
    tf2::fromMsg(odom.pose.pose, T_base_odom);
