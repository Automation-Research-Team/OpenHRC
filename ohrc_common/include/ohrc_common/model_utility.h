...
public:
model_utility(std::shared_ptr<rclcpp::Node> node) : node_(node){};
...
inline urdf::Model getURDFModel(const std::string URDF_param) {
  ...
  urdf_xml = node_->get_parameter("urdf_xml", URDF_param);
  // nh.searchParam(urdf_xml, full_urdf_xml);
  // node_->

  // ROS_DEBUG("Reading xml file from parameter server");
  if (!node_->get_parameter(full_urdf_xml, xml_string)) {
